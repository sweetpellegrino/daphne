...
// EwBinaryMat<CSRMatrix, CSCMatrix>
template<typename VT>
struct EwBinaryMat<CSRMatrix<VT>, CSRMatrix<VT>, CSCMatrix<VT>> {
    static void apply(BinaryOpCode opCode, CSRMatrix<VT> *& res, const CSRMatrix<VT> * lhs, const CSCMatrix<VT> * rhs, DCTX(ctx)) {
        const size_t numRows = lhs->getNumRows();
        const size_t numCols = lhs->getNumCols();
        if( numRows != rhs->getNumRows() || numCols != rhs->getNumCols() )
            throw std::runtime_error("EwBinaryMat(CSR) - lhs and rhs must have the same dimensions.");
