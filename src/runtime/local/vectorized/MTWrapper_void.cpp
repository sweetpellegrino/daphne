...
void MTWrapper<void>::executeCpuQueues(
        std::vector<std::function<MTWrapper<void>::PipelineFunc>> funcs, void ***res,
        const bool* isScalar, Structure **inputs, size_t numInputs, int64_t* outputTypes, size_t numOutputs, int64_t *outRows, int64_t *outCols,
    ...
    mem_required += this->allocateOutput(res, numOutputs, outputTypes, outRows, outCols, combines);
    ...
    if (ctx->getUserConfig().prePartitionRows) {
        ...
        if (ctx->getUserConfig().pinWorkers) {
            for(int i=0; i<this->_numQueues; i++) {
                while (lps[i].hasNextChunk()) {
                    ...
                    qvector[i]->enqueueTaskPinned(new CompiledPipelineTask<void>(CompiledPipelineTaskData<void>{funcs, isScalar,
                            inputs, numInputs, outputTypes, numOutputs, outRows, outCols, splits, combines, startChunk, endChunk, outRows,
            ...
            for(int i=0; i<this->_numQueues; i++) {
                while (lps[i].hasNextChunk()) {
                    endChunk += lps[i].getNextChunk();
                    qvector[i]->enqueueTask(new CompiledPipelineTask<void>(CompiledPipelineTaskData<void>{funcs, isScalar,
                            inputs, numInputs, outputTypes, numOutputs, outRows, outCols, splits, combines, startChunk, endChunk, outRows,
        ...
        if (ctx->getUserConfig().pinWorkers) {
            while (lp.hasNextChunk()) {
                ...
                qvector[target]->enqueueTaskPinned(new CompiledPipelineTask<void>(CompiledPipelineTaskData<void>{funcs, isScalar,
                        inputs, numInputs, outputTypes, numOutputs, outRows, outCols, splits, combines, startChunk, endChunk, outRows,
		...
		currentItr++;
            ...
            while (lp.hasNextChunk()) {
                endChunk += lp.getNextChunk();
                target = currentItr % this->_numQueues;
                qvector[target]->enqueueTask(new CompiledPipelineTask<void>(CompiledPipelineTaskData<void>{funcs, isScalar,
                        inputs, numInputs, outputTypes, numOutputs, outRows, outCols, splits, combines, startChunk, endChunk, outRows,
                        outCols, 0, ctx}, resLock, res));
                startChunk = endChunk;
		currentItr++;
