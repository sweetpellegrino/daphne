...
void Greedy1VectorizeComputationsPass::runOnOperation()
{
    ...
    std::unordered_set<FusionCandidate> producerConsumerCandidates;
    std::unordered_set<FusionCandidate> horizontalCandidates;
        ...
        for (size_t i = 0; i < vectOp->getNumOperands(); i++) {
            ...
            if(llvm::isa<daphne::MatrixType>(operand.getType())) {
                ...
                for (const auto &user : operand.getUsers()) {
                    ...
                    if (user == vectOp || //Does not make sense to consider the opv with itself
                        !vectUser || //User must be Vectorizable
                        user->getBlock() != vectOp->getBlock()) //TODO: To restrictive?
                        continue;
                    ...
                    if (is_only_horizontal) {
                        if (vectOp.getVectorSplits()[ZeroDecision][i] == vectUser.getVectorSplits()[ZeroDecision][userOperandIx]) {
                            ...
                            horizontalCandidates.insert({op, user, FusionCandidate::Type::HORIZONTAL});
                        }
                    }
                }
                ...
                if(auto vectProducer = llvm::dyn_cast<daphne::Vectorizable>(producer)) { 
                    ...
                    if(VectorUtils::matchingVectorSplitCombine(split, combine) && 
                        ...
                        producerConsumerCandidates.insert({producer, op, FusionCandidate::Type::PRODUCER_CONSUMER});
                    }
                }
            }
        }
    ...
    VectorUtils::DEBUG::printPipelines(ops, operationToPipelineIx, decisionIxs, "graph-gr1.dot");
